...
struct plqueue_u64_rw * queue_create (void) {
  ...
  *q = (struct plqueue_u64_rw) {
    .q = {
      ...
      .entries = calloc (1 << log2_capacity, sizeof(*q->q.entries))
    }
  };
  return q;
}
...
void queue_destroy (struct plqueue_u64_rw * q) {
  if (q) { free (q->q.entries); free (q); }
}

int queue_enqueue (struct plqueue_u64_rw * q, uint64_t payload) {
  return plqueue_mp_enqueue_u64 (&q->q, &q->write_cursor, payload);
}

int queue_dequeue (struct plqueue_u64_rw * q, uint64_t * payload) {
  struct maybe_payload_u64 r = plqueue_mc_dequeue_u64 (&q->q, &q->read_cursor);
  if (r.has_value) {
    *payload = r.payload;
    return 1;
  } else {
...
int queue_is_empty (struct plqueue_u64_rw * q) {
  return plqueue_mc_is_empty_u64 (&q->q, &q->read_cursor);
}
