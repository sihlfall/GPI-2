...
int
gaspi_sn_connect2port (const char *const hn, const unsigned short port,
...
gaspi_sn_connect_to_rank (const gaspi_rank_t rank,
                          const gaspi_timeout_t timeout_ms)
{
...
gaspi_sn_backend (void* GASPI_UNUSED (args))
{
  ...
  {
    gaspi_sn_fatal_error (lsock, GASPI_SN_STATE_ERROR,
                          "Failed to create IO event facility.");
    return NULL;
  }
  ...
  {
    gaspi_sn_fatal_error (lsock, GASPI_SN_STATE_ERROR,
                          "Failed to add fd to IO event facility.");
    return NULL;
  }
  ...
  while (!_gaspi_sn_stop)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      else if (mgmt->fd == lsock)
      {
        ...
        {
          gaspi_sn_fatal_error (nsock, GASPI_SN_STATE_ERROR,
                                "Failed to set socket options.");
          return NULL;
        }
            ...
            else
            {
              ...
              if (mgmt->bdone == mgmt->blen)
              {
                ...
                if (mgmt->op == GASPI_SN_HEADER)
                {
                  ...
                  {
                    if (GASPI_ENDPOINT_CONNECTED ==
                        gctx->ep_conn[mgmt->cdh.rank].cstat)
                    {
                  ...
                  {
                    /* These commands require more work: possibly
                     * reading more and a response (data or ack) */
                    ...
                    if (mgmt->cdh.op == GASPI_SN_GRP_CHECK)
                    {
                      gaspi_group_exch_info_t *gb =
                        pgaspi_group_create_exch_info (mgmt->cdh.rank,
                                                       mgmt->cdh.tnc);
                    ...
                    {
                      const gaspi_group_ctx_t *grp_to_connect =
                        &(gctx->groups[mgmt->cdh.ret]);
                    ...
                    {
                      gaspi_dev_exch_info_t *const exch_info =
                        &(gctx->ep_conn[mgmt->cdh.rank].exch_info);

                      gaspi_return_t eret =
                        pgaspi_create_endpoint_to (mgmt->cdh.rank, exch_info,
                                                   sn_config_timeout);
                      if (eret != GASPI_SUCCESS)
                      {
                        GASPI_DEBUG_PRINT_ERROR
                      ...
                      ssize_t info_read =
                        gaspi_sn_readn (mgmt->fd, exch_info->remote_info,
                                        exch_info->info_size);
                      ...
                      eret =
                        pgaspi_connect_endpoint_to (mgmt->cdh.rank,
                                                    sn_config_timeout);
                      if (eret != GASPI_SUCCESS)
                      {
                    ...
                    ssize_t wbytes =
                      gaspi_sn_writen (mgmt->fd, response, response_size);
                    if (response_free)
                    {
                      free (response);
                    }
